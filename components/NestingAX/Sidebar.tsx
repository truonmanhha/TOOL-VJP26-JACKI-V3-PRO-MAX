      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
        {nestLists.length === 0 ? (
          ...
          nestLists.map((list) => {
            ...
            const listParts = isActive ? parts : (list.parts || []);
                      ...
                      return (
                        <div key={part.id} className="mb-2">
                          <div 
                            ...
                            className={`flex items-start p-2 rounded-md border transition-all group cursor-pointer ${
                              isPartActive 
                          ...
                          >
                            <div className="flex flex-col items-center mr-3 shrink-0">
                              <div className={`text-[7px] font-black mb-1 tracking-tighter uppercase transition-colors ${isPartActive ? 'text-cyan-400' : 'text-slate-500'}`}>Preview 2</div>
                              <div className={`w-20 h-20 bg-[#020408] border rounded flex items-center justify-center overflow-hidden relative shadow-inner transition-colors ${
                                ...
                                {hasPoints ? (
                                  ...
                                  <div className="text-[8px] text-red-500 font-bold text-center leading-none uppercase">Selection<br/>Failed</div>
                                )}
                              </div>
                            </div>

                            <div className="overflow-hidden flex-1 flex flex-col h-20 justify-between py-0.5">
                              <div>
                                <div className={`font-bold text-[11px] truncate leading-tight mb-0.5 ${isPartActive ? 'text-cyan-300' : 'text-blue-100'}`} title={part.name}>{part.name}</div>
                                <div className="text-[9px] text-slate-500 font-mono italic">{part.dimensions}</div>
                              </div>
                              
                              <div className="flex items-center gap-2">
                                <div className="text-[9px] text-slate-300 bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700 flex items-center">
                                  <span className="text-slate-500 font-bold mr-1">QTY:</span>{part.required}
                                </div>
                                <div className="text-[9px] text-slate-300 bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700 flex items-center">
                                  <span className="text-slate-500 font-bold mr-1">PRI:</span>{part.priority}
                                </div>
                              </div>
                            </div>
                          </div>
