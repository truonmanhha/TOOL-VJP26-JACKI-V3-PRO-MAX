          ...
          <div className="glass-panel border-orange-500/20 p-8 rounded-3xl flex flex-col md:flex-row items-center gap-8 shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4"><span className="text-[10px] text-slate-500 font-black uppercase">Sai số vá</span><input type="number" min="0.01" max="10" step="0.01" value={joinTolerance} onChange={e => { let val = parseFloat(e.target.value); if(isNaN(val)) return; if(val<0.01) val=0.01; if(val>10) val=10; setJoinTolerance(val); }} className="w-16 bg-slate-200 border border-slate-400 rounded px-1 text-center text-black font-bold outline-none focus:border-orange-500 focus:bg-white text-[10px]" /><span className="text-sm font-mono text-orange-400 font-bold w-12">{joinTolerance}</span></div>
              </div>
            </div>
