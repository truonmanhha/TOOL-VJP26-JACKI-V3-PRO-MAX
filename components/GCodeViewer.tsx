        ...
        <div className="absolute top-20 right-6 z-40 flex flex-col items-end select-none pointer-events-auto">
             <div className="relative w-24 h-24 group">
                 ...
                 <div className="w-full h-full bg-slate-900/90 rounded-full border-[3px] border-slate-700/50 relative flex items-center justify-center shadow-2xl backdrop-blur-md">
                     ...
                     <button onClick={() => onSetView('TOP')} className="w-12 h-12 bg-slate-300 hover:bg-white text-slate-900 font-black text-[10px] flex items-center justify-center rounded-[4px] shadow-inner transition-all z-0 border-2 border-slate-400 hover:border-white hover:scale-105 active:scale-95">
                        TOP
                     </button>
...
const SceneContent: React.FC<{ 
    ...
    useEffect(() => { if (commands.length > 0 && !initRef.current) { fitView(); initRef.current = true; } }, [commands, fitView]);
...
const GCodeViewer: React.FC<GCodeViewerProps> = ({ lang, isLiteMode, setIsLiteMode }) => {
  ...
  const renderToolbarButtons = () => (
    <div className="flex items-center gap-2 md:gap-3 flex-wrap justify-center">
            <button onClick={() => setIsLiteMode(!isLiteMode)} className={`flex items-center gap-2 px-3 py-2 md:p-2 rounded-xl transition-all border border-transparent ${isLiteMode ? 'bg-amber-600/20 text-amber-400 border-amber-500/50' : 'bg-slate-800 text-slate-500 hover:text-white'}`} title={isLiteMode ? "Chế độ tối ưu (Lite Mode): ĐANG BẬT" : "Chế độ tối ưu (Lite Mode): ĐANG TẮT"}>{isLiteMode ? <Zap size={18} className="fill-amber-400" /> : <Gauge size={18} />}<span className="text-[10px] font-black uppercase hidden md:inline">{isLiteMode ? "GIAO DIỆN: TỐI ƯU" : "GIAO DIỆN: FULL"}</span></button>
            <button onClick={() => setShowGrid(!showGrid)} className={`p-3 md:p-2 rounded-xl transition-all ${showGrid ? 'bg-blue-600/20 text-blue-400' : 'bg-slate-800 text-slate-500'}`} title="Bật/Tắt Lưới">{showGrid ? <Eye size={18} /> : <EyeOff size={18} />}</button>
            <button onClick={() => setZoomFitTrigger(p => p + 1)} className="p-3 md:p-2 bg-slate-800 text-slate-500 hover:bg-slate-700 hover:text-white rounded-xl transition-all" title="Auto Fit"><Focus size={18} /></button>
            <div className="relative">
                <button onClick={() => setShowStarMenu(!showStarMenu)} className={`p-3 md:p-2 rounded-xl transition-all flex items-center gap-2 ${showStarMenu ? 'bg-purple-600 text-white' : 'bg-slate-800 text-slate-500'}`} title="Chế độ Sao"><Sparkles size={18} /> <ChevronDown size={14} /></button>
                {showStarMenu && <div className="absolute top-12 right-0 bg-slate-950 border border-white/10 rounded-xl p-2 w-64 shadow-2xl z-[100]"><div className="text-[10px] font-black uppercase text-slate-500 px-2 py-1 mb-1">CÔNG CỤ QUẢ CẦU</div>{Object.entries(StarMode).map(([key, label]) => { const mode = StarMode[key as keyof typeof StarMode]; return (<button key={key} onClick={() => { setStarMode(mode); setShowStarMenu(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${starMode === mode ? 'bg-purple-600/20 text-purple-400' : 'hover:bg-white/5 text-slate-300'}`}>{mode === StarMode.OFF ? <X size={14} /> : mode === StarMode.STAR_WARS ? <Swords size={14} /> : mode === StarMode.NORMAL ? <Globe size={14} /> : <Sparkles size={14} />}{label}</button>); })}</div>}
            </div>
            <div className="relative">
                <button onClick={() => setShowViewModeMenu(!showViewModeMenu)} className={`p-3 md:p-2 rounded-xl transition-all flex items-center gap-2 ${showViewModeMenu ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-500'}`} title="Chế độ hiển thị"><ListFilter size={18} /> <ChevronDown size={14} /></button>
                {showViewModeMenu && <div className="absolute top-12 right-0 bg-slate-950 border border-white/10 rounded-xl p-2 w-64 shadow-2xl z-[100]"><div className="text-[10px] font-black uppercase text-slate-500 px-2 py-1 mb-1">CHỌN KIỂU MÔ PHỎNG</div>{Object.values(ViewMode).map(mode => (<button key={mode} onClick={() => { setViewMode(mode); setShowViewModeMenu(false); }} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${viewMode === mode ? 'bg-blue-600/20 text-blue-400' : 'hover:bg-white/5 text-slate-300'}`}><div className={`w-2 h-2 rounded-full ${viewMode === mode ? 'bg-blue-500' : 'bg-slate-700'}`} />{mode}</button>))}</div>}
            </div>
            <div className="relative">
                <button onClick={() => setShowViewOptionsMenu(!showViewOptionsMenu)} className={`p-3 md:p-2 rounded-xl transition-all flex items-center gap-2 ${showViewOptionsMenu ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-500'}`} title="Tùy chọn hiển thị"><MoreHorizontal size={18} /></button>
                {showViewOptionsMenu && <div className="absolute top-12 right-0 bg-slate-950 border border-white/10 rounded-xl p-2 w-64 shadow-2xl z-[100]"><div className="text-[10px] font-black uppercase text-slate-500 px-2 py-1 mb-1">TÙY CHỌN HIỂN THỊ</div>{[{ key: 'showToolpath', label: 'Hiện đường chạy dao' }, { key: 'showPoints', label: 'Hiện các điểm' }, { key: 'showRapid', label: 'Hiện đường G0' }, { key: 'showCutting', label: 'Hiện đường G1/G2/G3' }, { key: 'highlightArcs', label: 'Tô màu cung G2/G3' }, { key: 'highlightElement', label: 'Highlight khi Hover' }, { key: 'largePoints', label: 'Điểm to hơn' }].map(opt => (<button key={opt.key} onClick={() => setViewOptions(prev => ({ ...prev, [opt.key]: !prev[opt.key as keyof ViewOptions] }))} className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-between ${viewOptions[opt.key as keyof ViewOptions] ? 'bg-blue-600/20 text-blue-400' : 'hover:bg-white/5 text-slate-400'}`}><span>{opt.label}</span>{viewOptions[opt.key as keyof ViewOptions] && <Check size={14} />}</button>))}</div>}
            </div>
            <div className="relative">
                <button onClick={() => setShowToolConfig(!showToolConfig)} className={`p-3 md:p-2 rounded-xl transition-all flex items-center gap-2 ${showToolConfig ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/50' : 'bg-slate-800 text-slate-500'}`} title="Cấu hình dao"><Wrench size={18} /></button>
                {showToolConfig && <div className="absolute top-12 right-0 bg-slate-950 border border-white/10 rounded-xl p-4 w-64 shadow-2xl z-[100]"><div className="flex justify-between items-center mb-4"><span className="text-xs font-black uppercase text-white">CẤU HÌNH DAO</span><button onClick={() => setShowToolConfig(false)}><X size={14} className="text-slate-500" /></button></div><div className="space-y-4">{[{ l: 'Đường kính mũi', k: 'diameter' }, { l: 'Chiều dài mũi', k: 'length' }, { l: 'Đường kính cán', k: 'holderDiameter' }, { l: 'Chiều dài cán', k: 'holderLength' }].map(f => (<div key={f.k}><label className="text-[10px] font-black text-slate-500 uppercase block mb-1">{f.l} (mm)</label><input type="number" value={toolConfig[f.k as keyof ToolConfig]} onChange={e => setToolConfig({ ...toolConfig, [f.k]: parseFloat(e.target.value) })} className="w-full bg-slate-800 border border-white/10 rounded-lg px-2 py-1.5 text-xs text-white outline-none" /></div>))}</div></div>}
            </div>
            <button onClick={() => { setSnapMode(!snapMode); setMeasurePoints([]); }} className={`p-3 md:p-2 rounded-xl transition-all flex items-center gap-2 ${snapMode ? 'bg-orange-600/20 text-orange-400 border border-orange-500/50' : 'bg-slate-800 text-slate-500'}`} title="Công cụ đo đạc"><Ruler size={18} /> <MousePointerClick size={14} /></button>
            <div className="relative"><button onClick={() => setShowThemeSettings(!showThemeSettings)} className="p-3 md:p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-all relative z-50"><Palette size={18} /></button>{showThemeSettings && <div className="absolute top-12 right-0 bg-slate-950 border border-white/10 rounded-xl p-4 w-72 shadow-2xl z-[100]"><div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2"><span className="text-xs font-black uppercase text-white tracking-widest">MÀU SẮC</span><button onClick={() => setShowThemeSettings(false)} className="hover:text-white text-slate-500 transition-colors"><X size={14} /></button></div><div className="grid grid-cols-3 gap-2 mb-4"><button onClick={() => setTheme(DEFAULT_THEME)} className="bg-slate-800 hover:bg-slate-700 text-[9px] font-bold text-slate-300 py-1.5 rounded-lg border border-white/5 transition-all">MẶC ĐỊNH</button><button onClick={() => setTheme({ ...DEFAULT_THEME, background: '#ffffff', grid: '#e2e8f0', g0: '#ef4444', g1: '#0f172a', arc: '#3b82f6', text: '#000000' })} className="bg-slate-200 hover:bg-white text-[9px] font-bold text-slate-900 py-1.5 rounded-lg border border-white/5 transition-all">SÁNG</button><button onClick={() => setTheme({ ...DEFAULT_THEME, background: '#172554', grid: '#1e3a8a', g0: '#facc15', g1: '#60a5fa', arc: '#c084fc', text: '#dbeafe' })} className="bg-blue-950 hover:bg-blue-900 text-[9px] font-bold text-blue-200 py-1.5 rounded-lg border border-blue-500/30 transition-all">BLUEPRINT</button></div><div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">{Object.entries(theme).map(([key, val]) => (<div key={key} className="flex items-center justify-between group"><span className="text-[10px] font-bold uppercase text-slate-400 group-hover:text-white transition-colors">{THEME_LABELS[key as keyof ThemeConfig]}</span><div className="flex items-center gap-2"><span className="text-[9px] font-mono text-slate-600 uppercase">{val}</span><div className="relative w-6 h-6 rounded-full overflow-hidden border border-white/20 shadow-sm cursor-pointer hover:scale-110 transition-transform"><input type="color" value={val} onChange={e => setTheme({ ...theme, [key]: e.target.value })} className="absolute -top-1/2 -left-1/2 w-[200%] h-[200%] cursor-pointer p-0 border-0" /></div></div></div>))}</div><div className="mt-4 pt-2 border-t border-white/10 text-center"><button onClick={() => setTheme(DEFAULT_THEME)} className="text-[9px] text-slate-500 hover:text-white flex items-center justify-center gap-1 mx-auto transition-colors"><RefreshCcw size={10} /> Khôi phục mặc định</button></div></div>}</div>
           {!isLiteMode && <button onClick={handleAIAnalyze} disabled={isAnalyzing || !content} className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white px-5 py-2 rounded-xl text-xs font-black flex items-center gap-2 shadow-[0_0_15px_rgba(168,85,247,0.4)] transition-all">{isAnalyzing ? <Activity className="animate-spin" size={14} /> : <Zap size={14} />} AI ANALYZE</button>}
    </div>
  );
