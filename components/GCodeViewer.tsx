...
const ViewCubeUI: React.FC<{ 
    ...
    return (
        <div className="absolute top-20 right-6 z-[9990] flex flex-col items-end select-none pointer-events-auto">
...
const GCodeViewer: React.FC<GCodeViewerProps> = ({ lang, isLiteMode, setIsLiteMode }) => {
  ...
  const ThreeDViewContent = (
      <>
          ...
          <button onClick={toggleFullScreen} className={`absolute right-4 z-[10000] p-2.5 ${is3DFullScreen ? "top-4 bg-blue-600/20 text-blue-400 border-blue-500/50" : "top-4 bg-slate-800/80 backdrop-blur-sm text-slate-400 hover:bg-blue-600 hover:text-white border-white/10"} rounded-xl transition-all active:scale-95 border shadow-xl`} title={is3DFullScreen ? "Thu nhỏ 3D" : "Phóng to 3D (2)"}>{is3DFullScreen ? <Minimize size={16} /> : <Maximize size={16} />}</button>

          <AnimatePresence>{snapMode && <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} className={`absolute right-4 sm:right-4 bg-orange-500/10 border border-orange-500/20 text-orange-400 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 pointer-events-none z-[9990] ${is3DFullScreen ? 'top-16' : 'top-24 sm:top-16'}`}><Ruler size={14} /> CHẾ ĐỘ ĐO ĐANG BẬT</motion.div>}</AnimatePresence>
      </>
  );
